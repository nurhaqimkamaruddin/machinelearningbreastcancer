...
class FlaskUI:
    ...
    def open_browser(self):
        ...
        if self.app_mode and self.fullscreen:
            self.BROWSER_PROCESS = sps.Popen(
                [self.browser_path, "--new-window", "--start-fullscreen", '--app={}'.format(self.localhost)],
                stdout=sps.PIPE, stderr=sps.PIPE, stdin=sps.PIPE)
        elif self.app_mode and self.maximized:
            self.BROWSER_PROCESS = sps.Popen(
                [self.browser_path, "--new-window", "--start-maximized", '--app={}'.format(self.localhost)],
                stdout=sps.PIPE, stderr=sps.PIPE, stdin=sps.PIPE)
        elif self.app_mode:
            self.BROWSER_PROCESS = sps.Popen(
                [self.browser_path, "--new-window", "--window-size={},{}".format(self.width, self.height),
